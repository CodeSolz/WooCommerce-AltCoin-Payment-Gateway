...
class wapgAdminNotice {
	...
	public static function upgrade() {
		...
		$message       = __( 'You are using free manual version. There is a pro version which can track your order and coin transfer automatically. Let\'s check the  %1$s trial version of pro plugin%2$s', 'woo-altcoin-payment-gateway' );
	...
	public static function is_trial_ended() {
		if ( get_option( 'wapg_pro_trial_ended' ) ) {
			$notice  = wapgNotice::get_instance();
			$message = __(
				'Your API has been expired. Automatic coin tracking not working anymore on your store.  If you were using trial version %1$sPurchase pro plan%2$s to re-activate automatic tracking or %1$sPurchase new plan%2$s to extend validity if you were already using pro package.
            To continue using free version, %3$sdelete the coins%2$s which has been listed as automatic from %3$sall coins list%2$s. ',
